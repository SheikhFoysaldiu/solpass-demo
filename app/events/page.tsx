      ...
      ? chainEvents
          .filter(
            (event) =>
              event &&
              event.account &&
              typeof event.account.eventId === "string"
          )
  ...
  const initializeEventMutation = useMutation({
    mutationFn: async ({
      ...
      try {
        ...
        const totalTicket = event.ticketTypes?.reduce((acc, ticketType) => {
          return acc + (Number(ticketType.available) || 0);
        }, 0);
    ...
    onSuccess: async (result, { event }) => {
      ...
      setEvents((prevEvents) =>
        prevEvents.map((e) =>
          e.id === event.id ? { ...e, chainEventKey: result.eventPda } : e
        )
      );
