...
export function generateDummyEvent() {
  const eventTypes = ["Concert", "Festival", "Conference", "Workshop", "Exhibition", "Sports Event", "Comedy Show"]
  const venues = [
    "Central Stadium",
    "City Arena",
    "Grand Hall",
  ...
  const artists = ["The Weeknd", "Taylor Swift", "BTS", "Ed Sheeran", "Billie Eilish", "Drake", "Adele", "Coldplay"]
  ...
  return {
    ...
    venue,
    image: `/placeholder.svg?height=400&width=600&text=${encodeURIComponent(name)}`,
      ...
      inventory: [
        {
          ...
          areas: [
            {
              description: index === 0 ? "General Admission" : "Reserved Seating",
              name: index === 0 ? "GA" : "RS",
              areaId: `${index}`,
